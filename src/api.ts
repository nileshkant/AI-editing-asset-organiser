...
export type Profile = {duration:number;sample_rate:number;channels:number;frames:number;peak:number;rms:number;description:string;tags:string[];waveform:[number,number][]};
...
export type Progress = {source_id:string;status:string;completed:number;total:number;reused:number;failed:number;current:string;errors:string[]};
