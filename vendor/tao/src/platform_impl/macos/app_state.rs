...
impl AppState {
  ...
  pub fn launched(app_delegate: &Object) {
    ...
    HANDLER.set_ready();
    HANDLER.waker().start();
    // macOS 26 may raise an Objective-C exception while Tauri initializes its
    // webview. Running the initial event after this callback returns prevents
    // that foreign exception from crossing the non-unwinding ObjC boundary.
